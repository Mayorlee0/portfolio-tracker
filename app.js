...
function getSupabasePublicConfig() {
  ...
  const paramUrl = params.get("supabaseUrl");
  const paramAnonKey = params.get("supabaseAnonKey");

  if (paramUrl && paramAnonKey) {
    return { url: paramUrl, anonKey: paramAnonKey };
  }

  if (window.PORTFOLIO_SUPABASE_CONFIG?.url && window.PORTFOLIO_SUPABASE_CONFIG?.anonKey) {
    return {
      url: window.PORTFOLIO_SUPABASE_CONFIG.url,
      anonKey: window.PORTFOLIO_SUPABASE_CONFIG.anonKey
    };
  }

  return DEFAULT_SUPABASE_PUBLIC_CONFIG;
}
...
const appBuildBadge = document.getElementById("appBuildBadge");
...
if (appBuildBadge) appBuildBadge.textContent = `build ${APP_VERSION}`;
...
async function connectBackend() {
  ...
  const activeSupabaseConfig = getSupabasePublicConfig();
  const client = window.supabase.createClient(activeSupabaseConfig.url, activeSupabaseConfig.anonKey);
  ...
  try {
    ...
    if (error) {
      ...
      const networkLike = /failed to fetch|network/i.test(message);
      if (networkLike) {
        setBackendStatus("Supabase auth request failed to reach your project. Verify Supabase URL/anon key and project availability.", true);
      } else {
        setBackendStatus(`Supabase reachable, but anonymous auth is disabled (${message}). Backend writes are blocked until auth is enabled.`, true);
      }
      console.error("[backend] auth_failed", message);
      updateRuntimeDebug();
