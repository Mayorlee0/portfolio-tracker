...
async function fetchLiveStockPrices() {
  ...
  const fallbackResults = await Promise.all(STOCK_CATALOG.map(fetchStockViaStooq));
  updated += fallbackResults.filter(Boolean).length;

  if (updated === 0) {
    throw new Error("Stock adapter failed (Yahoo unavailable and Stooq fallback returned no data)");
  }
}
